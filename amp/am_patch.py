...
def _install_package_shim() -> None:
    package_root = Path(__file__).resolve().parent / "am_patch"
    if __name__ == "__main__":
        import types

        pkg = sys.modules.get("am_patch")
        if pkg is None:
            pkg = types.ModuleType("am_patch")
            pkg.__file__ = str(Path(__file__).resolve())
            pkg.__package__ = "am_patch"
            sys.modules["am_patch"] = pkg
        pkg.__path__ = [str(package_root)]
        return
    if __name__ == "am_patch":
...
# the runner specification under amp/ (e.g., amp/am_patch_specification.md).
