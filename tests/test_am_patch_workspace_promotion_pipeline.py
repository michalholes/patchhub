...
def test_workspace_promotion_pipeline_cleans_failure_zips_once(tmp_path: Path) -> None:
    ...
    plan = plan_cls(
        files_to_promote=["amp/am_patch/engine.py"],
        issue_diff_base_sha="abc123",
        issue_diff_paths=["amp/am_patch/engine.py"],
        files_for_fail_zip=["amp/am_patch/engine.py"],
    )
    ...
    assert summary.final_pushed_files == ["M amp/am_patch/engine.py"]
