...
def test_single_run_gates_callsite_in_standalone_amp() -> None:
    repo_root = _repo_root()
    am_patch_dir = repo_root / "amp" / "am_patch"
    assert am_patch_dir.is_dir(), f"missing am_patch dir: {am_patch_dir}"

    allowed = "amp/am_patch/gates_policy_wiring.py"
...
def test_pytest_bucket_routing_external_callsite_is_only_gates_py() -> None:
    ...
    for py in _iter_py_files(am_patch_dir):
        rel = py.relative_to(repo_root).as_posix()
        if rel == "amp/am_patch/pytest_bucket_routing.py":
            continue
