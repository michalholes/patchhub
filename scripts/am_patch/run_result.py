...
from am_patch.workspace import Workspace, WorkspaceCheckpoint
...
def _new_blob_list() -> list[tuple[str, bytes]]:
    return []


def _new_failure_pairs() -> list[tuple[str, str]]:
    return []


def _stage_sort_key(*, stage_name: str, stage_rank: Callable[[str], int]) -> tuple[int, str]:
    return stage_rank(stage_name), stage_name
...
class RunResult:
    ...
    files_for_fail_zip: list[str] = field(default_factory=_new_str_list)
    failed_patch_blobs_for_zip: list[tuple[str, bytes]] = field(default_factory=_new_blob_list)
    patch_applied_successfully: bool = False
    applied_ok_count: int = 0
    rollback_ckpt_for_posthook: WorkspaceCheckpoint | None = None
    rollback_ws_for_posthook: Workspace | None = None
    issue_diff_base_sha: str | None = None
    issue_diff_paths: list[str] = field(default_factory=_new_str_list)
    delete_workspace_after_archive: bool = False
    ws_for_posthook: Workspace | None = None
    ...
    secondary_failures: list[tuple[str, str]] = field(default_factory=_new_failure_pairs)
...
def build_run_result(
    *,
    ...
    rollback_ckpt_for_posthook: WorkspaceCheckpoint | None,
    rollback_ws_for_posthook: Workspace | None,
    issue_diff_base_sha: str | None,
    issue_diff_paths: list[str],
    delete_workspace_after_archive: bool,
    ws_for_posthook: Workspace | None,
...
def normalize_failure_summary(
    *,
    error: RunnerError,
    primary_fail_stage: str | None,
    secondary_failures: list[tuple[str, str]],
    parse_gate_list: Callable[[str], list[str]],
    ...
    def _sort_key(stage_name: str) -> tuple[int, str]:
        return _stage_sort_key(stage_name=stage_name, stage_rank=stage_rank)
