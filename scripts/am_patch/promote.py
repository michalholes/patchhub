...
def promote_files(
    *,
    ...
    logger.info_core(
        f"promotion=START base_sha={base_sha} files={len(files_to_promote)}"
    )
    ...
    if files_to_promote:
        changed_live = git_ops.files_changed_since(
            logger, live_repo, base_sha, files_to_promote
        )
    ...
    if changed_live:
        ...
        if live_changed_resolution == "overwrite_workspace":
            ...
            files_to_promote = [
                f for f in files_to_promote if f not in set(changed_live)
            ]
